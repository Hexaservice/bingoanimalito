...
describe('finalización autoritativa de sorteo', () => {
  ...
  test('finaliza de inmediato al completar totalResultadosRequeridos tras persistir resultados', async () => {
    jest.resetModules();
    const { executeImmediateFinalizationAfterResultPersistence } = require('../uploadServer.js');

    const sorteoRef = { __kind: 'sorteo-ref', id: 's-inmediato' };
    const cantosRef = { __kind: 'cantos-ref', id: 's-inmediato' };

    const state = {
      sorteo: {
        estado: 'Jugando',
        formas: [{ idx: 1, nombre: 'Línea' }],
        ganadoresBloqueadosPorForma: {},
    ...
    const db = {
      collection: jest.fn((name) => {
        ...
        if (name === 'cantos') return { doc: jest.fn(() => cantosRef) };
        if (name === 'cantarsorteos') return { doc: jest.fn(() => ({ __kind: 'cantarsorteos-ref', id: 's-inmediato' })) };
        if (name === 'GanadoresSorteosTiempoReal') return { where: jest.fn(() => ({ __kind: 'winner-query' })) };
        return { doc: jest.fn(() => ({ __kind: 'doc-ref', id: 'x' })) };
      }),
      runTransaction: jest.fn(async (fn) => {
        const tx = {
          get: jest.fn(async (ref) => {
            if (ref === sorteoRef) return { exists: true, data: () => ({ ...state.sorteo }) };
            if (ref === cantosRef) return { exists: true, data: () => ({ ...state.cantos }) };
            if (ref?.__kind === 'winner-query') return { forEach: () => {} };
            if (ref?.__kind === 'cantarsorteos-ref') return { exists: true, data: () => ({ ...state.cantarsorteos }) };
            return { exists: false, data: () => ({}) };
          }),
          update: jest.fn((ref, payload) => {
            if (ref === sorteoRef) {
          ...
          set: jest.fn((ref, payload) => {
            if (ref?.__kind === 'cantarsorteos-ref') {
              state.cantarsorteos = { ...state.cantarsorteos, ...payload };
            }
          })
        };
        return fn(tx);
    ...
    const primerIntento = await executeImmediateFinalizationAfterResultPersistence({
      db,
      sorteoId: 's-inmediato',
      operadorEmail: 'operador@demo.com'
    });
    expect(primerIntento.finalized).toBe(false);
    expect(primerIntento.finalization?.motivo).toBe('faltan_resultados_y_ganadores');

    state.cantos.resultadosPorCelda = {
      'l1_08:00': { exacta: '17' }
    };

    const segundoIntento = await executeImmediateFinalizationAfterResultPersistence({
      db,
      sorteoId: 's-inmediato',
      operadorEmail: 'operador@demo.com'
    });
    expect(segundoIntento.finalized).toBe(true);
    expect(segundoIntento.finalization?.detalle?.totalResultadosRequeridos).toBe(1);
    expect(segundoIntento.finalization?.detalle?.totalResultadosCargados).toBe(1);
    expect(state.sorteo.estado).toBe('Finalizado');
    expect(state.sorteo.resultadoPublicadoJugadores).toBe('si');
    expect(state.sorteo.pdfresul).toBe('si');
    expect(state.cantarsorteos.acumuladoFormas?.montoCreditos || 0).toBe(0);
  });
});
